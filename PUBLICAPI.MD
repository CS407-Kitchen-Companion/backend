# API Documentation (REST / CRUD)

## Notes

- Auth `Y?` means contend differs depending on login status

# User API (/user)

## Table of Contents

| Name        | Link                             | Description              |
|-------------|----------------------------------|--------------------------|
| User API    | [User API](#User-Public-API)     | User Related Functions   |
| Recipes API | [Recipe API](#Recipe-Public-API) | Recipe Related Functions |
| Ratings API | [Rating API](#Rating-Public-API) | Rating Related Functions |

## API Endpoints

| Name          | Link                                                           | Description                                           |
|---------------|----------------------------------------------------------------|-------------------------------------------------------|
| Dev Endpoint  | `https://kitchencompanion.eastus.cloudapp.azure.com/api/v1/`   | Actual deployed/running SB Server                     |
| Mock Endpoint | `https://kitchencompanion.eastus.cloudapp.azure.com/api/mock/` | Python Django mock server (faster features, usually?) |

## User Public API

| Method | Mapping          | Description                        | Model/Body/Params                      | Auth | Status |
|--------|------------------|------------------------------------|----------------------------------------|------|--------|
| POST   | `/new`           | Used in new user Creation          | See [new user](#new-user)              | N    | Active |
| POST   | `/login`         | Used in user login                 | See [user login](#user-login)          | N    | Active |
| GET    | `/{id}`          | Get User Data                      | Params: uid, See [get user](#get-user) | N    | Active |
| GET    | `/{id}/username` | Get User Username Only             | Params: uid                            | N    | Active |
| GET    | `/saved`         | Get recipes saved by user          | See [user saved](#user-saved)          | Y    | Active |
| GET    | `/verify`        | Verify User email given token      | Params: uid, token                     | N    | Later  |
| GET    | `/all`           | Get information for all users      |                                        | N    | Later  |
| GET    | `/{id}/groups`   | Get list of groups created by user |                                        | N    | Later  |
| GET    | `/current`       | Get current user information       |                                        | Y?   | Later  | 
| DELETE | `/delete`        | Delete the current logged in user  |                                        | Y    | Later  | 

### User Info API (Other)

| Method | Mapping          | Description            | Model/Body/Params | Auth | Status |
|--------|------------------|------------------------|-------------------|------|--------|
| GET    | `/{id}/username` | Get User Username Only | Params: uid       | N    | Active |

### User Development API

| Method | Mapping     | Description                          | Model/Body/Params | Auth |
|--------|-------------|--------------------------------------|-------------------|------|
| GET    | `/testauth` | Development use check logged in user |                   | Y?   |

## User API Models

### new user

#### Request Body

```json
{
  "username": "username",
  "password": "password",
  "email": "email"
}
```

#### Response Body

```json
{
  "response": "Please Verify Email",
  "status": 200,
  "error": false
}
```

### user login

#### Request Body

```json
{
  "username": "test_2",
  "password": "test_2_pass"
}
```

#### Response Body

```json
{
  "token": "eyJhbGciOiJIUzUxMiJ9..."
}
```

### get user

#### Request Body

N/A, use path: `/user/{id}`

#### Response Body

```json
{
  "response": "See Data.",
  "status": 200,
  "error": false,
  "data": {
    "id": 1,
    "username": "test_1",
    "email": "test@email",
    "createdAt": "2024-02-12T21:05:41.846385Z"
  }
}
```

### user saved

#### Request Body

N/A, use path: `/saved`

#### Response Body

```json
{
  "response": "See Data.",
  "status": 200,
  "error": false,
  "data": [
    2,
    3,
    4
  ]
}
```

## Recipe Public API

| Method | Mapping      | Description                 | Model/Body/Params                         | Auth | Status |
|--------|--------------|-----------------------------|-------------------------------------------|------|--------|
| POST   | `/new`       | Used in new Recipe Creation | See [new recipe](#new-recipe)             | Y    | Active |
| GET    | `/{id}`      | Used to get a Recipe        | See [get recipe](#get-recipe)             | N    | Active |
| GET    | `/get`       | Used to get several Recipes | See [get recipe multi](#get-recipe-multi) | N    | Active |
| GET    | `/{id}/save` | Used to get a Recipe        | Save a post as a user                     | Y    | Active |

## Recipe API Models

### new recipe

#### Request Body

Calories optional value

```json
{
  "title": "title 1",
  "content": [
    "direction1",
    "direction2"
  ],
  "serves": 5,
  "time": 3600,
  "calories": 105,
  "tags": [
    "abc",
    "def",
    "tag1"
  ],
  "appliances": [
    "Knife",
    "Litteral_Kitchen"
  ],
  "ingredients": {
    "i1": "p1",
    "i2": "p2"
  }
}
```

#### Response Body

```json
{
  "response": "See Data.",
  "status": 200,
  "error": false,
  "data": {
    "id": 3,
    "title": "title 1",
    "content": [
      "direction1",
      "direction2"
    ],
    "createdBy": 2,
    "createdAt": "2024-02-20T21:18:58.875501Z",
    "comments": [],
    "ratings": [],
    "appliances": [
      "Knife",
      "Litteral_Kitchen"
    ],
    "tags": [
      "abc",
      "def",
      "tag1"
    ],
    "ingredients": {
      "i1": "p1",
      "i2": "p2"
    },
    "ratingCount": null,
    "calculatedRating": null,
    "serves": 5,
    "time": 3600,
    "calories": null,
    "updatedAt": "2024-02-20T21:18:58.875501Z",
    "edited": false
  }
}
```

### get recipe

#### Request Body

N/A, use path: `/recipe/{id}`

#### Response Body

```json
{
  "response": "See Data.",
  "status": 200,
  "error": false,
  "data": {
    "id": 24,
    "title": "title 1",
    "content": [
      "direction1",
      "direction2"
    ],
    "createdBy": 2,
    "createdAt": "2024-02-20T23:43:38.226555Z",
    "comments": [],
    "ratings": [
      2
    ],
    "appliances": [
      "Knife",
      "Literal_Kitchen"
    ],
    "tags": [
      "abc",
      "def",
      "tag1"
    ],
    "ingredients": {
      "i1": "p1",
      "i2": "p2"
    },
    "ratingCount": 1,
    "calculatedRating": 5,
    "serves": 5,
    "time": 3600,
    "calories": 105,
    "updatedAt": "2024-02-20T23:44:21.954814Z",
    "edited": false
  }
}
```

### get recipe multi

#### Request Body
```json
{
  "recipes": [
    1,
    2,
    3
  ]
}
```
#### Response Body

```json
{
  "response": "See Data.",
  "status": 200,
  "error": false,
  "data": [
    ...
  ]
}
```

## Rating Public API

| Method | Mapping | Description                 | Model/Body/Params             | Auth | Status |
|--------|---------|-----------------------------|-------------------------------|------|--------|
| POST   | `/new`  | Used in new Rating Creation | See [new rating](#new-rating) | Y    | Active |
| GET    | `/{id}` | Used to get a Rating        | See [get rating](#get-rating) | N    | Active |

## Rating API Models

### new rating

#### Request Body

```json
{
  "recipe_id": 1,
  "rating": 5
}
```

#### Response Body

```json
{
  "response": "See Data.",
  "status": 201,
  "error": false,
  "data": {
    "id": 1,
    "recipe": 1,
    "createdBy": 2,
    "rating": 5
  }
}
```

### get rating

#### Request Body

N/A, use path: `/rating/{id}`

#### Response Body

```json
{
  "response": "See Data.",
  "status": 200,
  "error": false,
  "data": {
    "id": 1,
    "recipe": 1,
    "createdBy": 2,
    "rating": 5
  }
}
```

## Search Public API

| Method | Mapping                                             | Description                                   | Model/Body/Params                             | Auth | Status |
|--------|-----------------------------------------------------|-----------------------------------------------|-----------------------------------------------|------|--------|
| GET    | `/recipe/search?title=&calories=&appliances=&tags=` | Used to search for recipes                    | See [get search](#get-search)                 | N    | Active |
| GET    | `/recipe/search/titles?partialTitle=Spa`            | Used to search for recipes with partial title | See [get search partial](#get-search-partial) | N    | Active |

### get search

#### Request Body

N/A, use path: `/recipe/search?title=Spa&calories=400&appliances=Large skillet`

title is partial title so Spa will give Spaghetti

calories will give all calories below mentioned number

appliances have to be exactly string matched
#### Response Body

```json
{
  "response": "See Data.",
  "status": 200,
  "error": false,
  "data": [
    {
      "id": 1,
      "title": "Spaghetti Recipe",
      "content": [
        "Cook spaghetti according to package instructions until al dente.",
        "In a large skillet, heat olive oil over medium heat.",
        "Add minced garlic and sauté until fragrant.",
        "Add diced tomatoes, tomato paste, and Italian seasoning. Cook until tomatoes break down and sauce thickens, about 10 minutes.",
        "Season with salt and pepper to taste.",
        "Add cooked spaghetti to the skillet and toss to coat in the sauce.",
        "Serve hot, garnished with grated Parmesan cheese and chopped fresh basil."
      ],
      "createdBy": 1,
      "createdAt": "2024-02-22T00:30:13.405327Z",
      "comments": [],
      "ratings": [
        1,
        2,
        3,
        4
      ],
      "appliances": [
        "Large skillet",
        "Pot for boiling water"
      ],
      "tags": [
        "spaghetti",
        "Italian",
        "pasta"
      ],
      "ingredients": {
        "garlic": "3 cloves, minced",
        "salt": "to taste",
        "spaghetti": "1 pound",
        "Italian seasoning": "1 teaspoon",
        "black pepper": "to taste",
        "Parmesan cheese": "for garnish",
        "diced tomatoes": "1 can (14 oz)",
        "fresh basil": "for garnish",
        "tomato paste": "2 tablespoons",
        "olive oil": "2 tablespoons"
      },
      "ratingCount": 4,
      "calculatedRating": 2,
      "serves": 4,
      "time": 20,
      "calories": 350,
      "updatedAt": "2024-02-22T02:02:22.219862Z",
      "edited": false
    }
  ]
}
```
### get search partial

#### Request Body

N/A, use path: `/recipe/search/titles?partialTitle=Spa`

#### Response Body

```json
{
  "response": "See Data.",
  "status": 200,
  "error": false,
  "data": [
    "Spaghetti Recipe"
  ]
}
```
# Anything further is not valid

## Discussion API (/discussions)

### Discussion Public API

| Method | Mapping        | Description                                          | Model/Body/Params    | Auth | Status |
|--------|----------------|------------------------------------------------------|----------------------|------|--------|
| POST   | `/new`         | Used in new discussion creation                      | See `DiscussionNew`  | Y    | Active |
| PUT    | `/{id}`        | Edit discussion information                          | See `DiscussionEdit` | Y    | Active | 
| GET    | `/all`         | Get list of all discussions                          |                      | N    | Active |
| GET    | `/{id}`        | Get information for specified discussion             |                      | N    | DEPREC | 
| GET    | `/get`         | Get information for one or more discussions          |                      | N    | DEV-A  |
| GET    | `/{id}/posts`  | Get list of all posts created in discussion          |                      | N    | DEPREC |
| GET    | `/{id}/posts2` | Get list of all posts created in discussion (faster) |                      | N    | DEV-A  |
| DELETE | `/{id}`        | Delete the given discussion                          |                      | Y    | Active |

### Discussion API Models

#### DiscussionNew

```json
{
  "title": "title",
  "description": "description"
}
```

#### DiscussionEdit

```json
{
  "title": "title",
  "description": "description"
}
```

#### DiscussionGet(V2)

URL param

| Variable | Optional | Description                        |
|----------|----------|------------------------------------|
| list     | No       | Query Param, used for many at once |

Example usage:
`http://localhost:8080/discussion/get?list=23,24`

### Post Public API

| Method | Mapping          | Description                               | Model/Body/Params    | Auth | Status   |
|--------|------------------|-------------------------------------------|----------------------|------|----------|
| POST   | `/new`           | Used in new post creation                 | See `DiscussionNew`  | Y    | Active   |
| PUT    | `/{id}`          | Edit post information                     | See `DiscussionEdit` | Y    | Active   | 
| GET    | `/all`           | Get list of all post                      |                      | N    | Active   |
| GET    | `/{id}`          | Get information for specified post        |                      | N    | DEPREC   | 
| GET    | `/get`           | Get information for one or more posts     |                      | N    | DEV-A    |
| GET    | `/{id}/comments` | Get list of all comments created in posts |                      | N    | NEWDEV-B |
| DELETE | `/{id}`          | Delete the given post                     |                      | Y    | Active   |

### Discussion API Models